...
```
Since bandit27-do allows us to run a command as another user, we can use it to gain the correct acces to run the command `cat /etc/bandit_pass/bandit27` (originally this gives us a Permission Denied message).
```console
...
The password to the next level is: Yz9IpL0sBcCeuG7m9uQFt8ZNpS4HZRc
```
