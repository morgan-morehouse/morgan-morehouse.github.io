...
### Level 1 &#8594; 2
**NOTE: To end each previous level, type in `exit` to log out. To start each new level, use the same ssh format as before (`ssh bandit0@bandit.labs.overthewire.org -p 2220`), but replace the zero in `bandit0` with the number of the level you are trying to access.**
