...
```
using `nano` I pasted in the RSA private key, then saved it (Crtl+O, Ctrl+X). Lastly, we make the permissions for sshkey17 600 and loginto level 17 with it. 
```console
...
```
Then we find the actual password for level 17 in case we need to login in another time.
```console
